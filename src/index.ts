...
const uniqueId = () => Math.random().toString(36).substring(2, 32);
...
async function __deleteCache(ws: WebSocket, key: string): Promise<void> {
    await _request(ws, {
        url: `db://delete:${key}`,
        method: "delete",
    });
...
async function _browserEvaluate(ws: WebSocket, script: string): Promise<string> {
...
function __setBrowserCookie(ws: WebSocket, url: string, cookie: string) {
    const data: WSSDataModel = {
...
async function __getBrowserCookie(ws: WebSocket, url: string): Promise<string> {
...
function _browserClick(ws: WebSocket, x: number, y: number) {
    const data: WSSDataModel = {
        ...
        if (wss?.action === WSSAction.browser_url_request) {
            ...
            if (props.onNavigationFinished) {
                props.onNavigationFinished(wss.data.url, {
                    evaluate: (script) => _browserEvaluate(ws, script),
                    click: (x, y) => _browserClick(ws, x, y),
                });
            }
        }
    }
            ...
            if (wss_data) {
                if (wss_data.action === WSSAction.stream) {
                    props.onStream({
                        ...
                        request: request,
                        controller: {
                            request: (props) => _request(ws, props),
                            ...
                            cache: {
                                ...
                                delete: (key) => __deleteCache(ws, key),
                            },
                            cookie: {
                                get: (url) => __getBrowserCookie(ws, url),
                                set: async (url, cookie) => __setBrowserCookie(ws, url, cookie),
                            },
                        },
                    });
                }
